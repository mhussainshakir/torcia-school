...
export async function POST(request: NextRequest) {
  try {
    ...
    const folderName = `Torcia_${folder || 'images'}`;
    const folderId = await createOrGetFolder(apiKey, folderName);
    ...
    const multipartBody = Buffer.concat([
      ...
      Buffer.from(delimiter),
      Buffer.from(`Content-Type: ${file.type || 'image/jpeg'}\r\n\r\n`),
      fileContent,
...
async function createOrGetFolder(apiKey: string, folderName: string): Promise<string | null> {
  try {
